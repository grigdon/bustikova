...
cor_matrix_hu <- cor(data_hu, method = "spearman", use = "pairwise.complete.obs")

# Get original column names to verify length
original_names <- colnames(cor_matrix_hu)
...
colnames(cor_matrix_hu) <- custom_labels
rownames(cor_matrix_hu) <- custom_labels

# Create a correlation heatmap and define output directory

output_dir = "~/projects/bustikova/output/corr"

pdf(file = file.path(output_dir, "hungary_corr.pdf"), width = 8, height = 6)
corr_plot <- corrplot(cor_matrix_hu, 
                      ...
                      title = "Interdependencies Between Hungarian Explanatory Factors: Spearman Correlation",
