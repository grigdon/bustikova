...
h <- data.frame(
  "Strongly Disagree" = apply(data_cz_questions[1:6], 2, prop, y = 1),
  "Disagree"          = apply(data_cz_questions[1:6], 2, prop, y = 2),
  "Agree"             = apply(data_cz_questions[1:6], 2, prop, y = 3),
  "Strongly Agree"    = apply(data_cz_questions[1:6], 2, prop, y = 4)
)
...
ggsave(filename = "~/projects/bustikova/output/avg_treat/survey_response_distribution/czechia_stacked_bar_graph.png", 
       plot = stacked_plot,
...
ggsave(filename = "~/projects/bustikova/output/avg_treat/average_treatment_standard/czechia_compare_bar_graph.png", 
...
control_means_vec <- c(
  A = mean(data_cz_questions$Control_A, na.rm = TRUE),
  B = mean(data_cz_questions$Control_B, na.rm = TRUE),
  C = mean(data_cz_questions$Control_C, na.rm = TRUE)
)

experimental_means_vec <- c(
  A = mean(data_cz_questions$Experimental_A, na.rm = TRUE),
  B = mean(data_cz_questions$Experimental_B, na.rm = TRUE),
  C = mean(data_cz_questions$Experimental_C, na.rm = TRUE)
)
...
ggsave(filename = "~/projects/bustikova/output/avg_treat/average_treatment_difference/czechia_difference_plot.png", 
...
pdf("~/projects/bustikova/output/avg_treat/numerical_results/czechia_numerical_results.pdf", width = 11, height = 8.5)
...
sample_sizes <- data.frame(
  ...
  N = c(
    sum(!is.na(data_cz_questions$Control_A)),
    sum(!is.na(data_cz_questions$Control_B)),
    sum(!is.na(data_cz_questions$Control_C)),
    sum(!is.na(data_cz_questions$Experimental_A)),
    sum(!is.na(data_cz_questions$Experimental_B)),
    sum(!is.na(data_cz_questions$Experimental_C))
  )
)
